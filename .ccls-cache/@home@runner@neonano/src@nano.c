...
void print_opt(const char *shortflag, const char *longflag,
...
void usage(void) {
  ...
  printf(_("To place the cursor on a specific line of a file, put the line "
           "number with\n"
           "a '+' before the filename.  The column number can be added after a "
           "comma.\n"));
...
bool wanted_to_move(void (*func)(void)) {
  return (func == do_left || func == do_right || func == do_up ||
          ...
          func == do_page_up || func == do_page_down || func == to_first_line ||
          func == to_last_line);
}
...
bool changes_something(functionptrtype f) {
  return (f == do_savefile || f == do_writeout || f == do_enter ||
          f == do_tab || f == do_delete || f == do_backspace || f == cut_text ||
          f == paste_text ||
#ifndef NANO_TINY
          f == chop_previous_word || f == chop_next_word || f == zap_text ||
          f == cut_till_eof || f == do_execute || f == do_indent ||
          f == do_unindent ||
#endif
#ifdef ENABLE_JUSTIFY
...
char **get_completions() {
  static char *sans[] = {"print", "printf", "preent", NULL};
  return sans;
}
bool cac(const char *str, const char *filter) {
  for (int i = 0; filter[i] != '\0'; i++) {
    if (!strchr(str, filter[i])) {
      return false;
    }
  }
  return true;
...
void filter_completion(char **arr, const char *filter, char ***result) {
  int count = 0;
  char **tempArr = arr;
  while (*tempArr) {
    if (cac(*tempArr, filter)) {
      count++;
    }
    tempArr++;
  }
...
void process_a_keystroke(void) {
  ...
  char sanskok[20];
  sprintf(sanskok, "%d", input);
  statusbar(sanskok);
  bool isUpOrDown = TRUE;
  if (!completion && (input == 259 || input == 258))
    isUpOrDown = FALSE;
  if (completion) {
    werase(completion);
    wrefresh(midwin);
  ...
  if (input == 263 || input == 127) {
    if (currindex > 0) {
      currword[--currindex] = '\0';
      currindex--;
    }
  } else if (input == 259 && currselected != 1)
    currselected--;
  else if (input == 258)
    currselected++;
  else if (input == 13 || input == 32 || input == 9 || !isalpha(input)) {
    currword[0] = '\0';
    currindex = 0;
    firstcharpos = gx;
    isUpOrDown = FALSE;
    ...
    currword[currindex++] = input;
    currword[currindex] = '\0';
  }
  // statusbar(currword);
  if (isUpOrDown) {
    char **sans = get_completions();
    char **res;
    filter_completion(sans, currword, &res);
    sans = res;
    if (sans[0]) {
      int height = 10;
      completion = newwin(height, COLS * 0.50, gy + 2, firstcharpos);
      box(completion, 0, 0);
      int maxt = 0;
      while (sans[maxt])
        maxt++;
      if (currselected > maxt)
        currselected = maxt;
      for (int i = 1; i < height - 1 && sans[i - 1]; i++) {
        sprintf(sanskok, "cursel: %d", currselected);
        statusbar(sanskok);
        if (i == currselected)
          wattron(completion, A_REVERSE);
        mvwprintw(completion, i, 1, "%s", sans[i - 1]);
        wattroff(completion, A_REVERSE);
      }
  ...
  if (ISSET(STATEFLAGS) && openfile->mark != was_mark)
    titlebar(NULL);
#endif
  printf("test test amognius");
  statusbar("ihis si a  tests");
  sprintf(sanskok, "isupordown: %d", isUpOrDown);
  statusbar(sanskok);
  if (isUpOrDown)
    wmove(midwin, gy, gx);
}
...
#define NOTREBOUND TRUE
  ...
  if (*openfile->filename == '\0' && openfile->totsize == 0 &&
    ...
    statusbar(_("Welcome to neonano.  For basic help, type Ctrl+G."));
#endif
